...
object HotcellAnalysis {
  ...
  Logger.getLogger("com").setLevel(Level.WARN)
    ...
    val avg = {
  val totalHotness = hotnessOfCellsDf
	    .agg(sum("cell_hotness").as("total"))
	    .collect()(0)
	    .getLong(0)
	    .toDouble

	  totalHotness / numCells
	}

    // calculate Standard deviation
    val stdDev = {
	  val squaredDF = hotnessOfCellsDf.withColumn("sqr_cell", pow(col("cell_hotness"), 2))
	  val totalSquared = squaredDF
	    .agg(sum("sqr_cell").as("sum_sqr"))
	    .collect()(0)
	    .getDouble(0)

	  val meanSquared = totalSquared / numCells
	  math.sqrt(meanSquared - (avg * avg))
	}
    ...
    val enrichedGScoreDF = sumOfAdjacentCellHotness.withColumn(
	  "gScore",
	  gScoreFunc(
	    lit(numCells),
	    col("x"), col("y"), col("z"),
	    col("AdjacentCellHotness"),
	    col("cellNumber"),
	    lit(avg),
	    lit(stdDev)
	  )
	...
	val gScoreHotCell = enrichedGScoreDF
